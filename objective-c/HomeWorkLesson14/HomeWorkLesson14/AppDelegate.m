...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [self masterWithSentencesWithNSString:[self studentWithArrayOfSentences:[self pupil]]];
...
//    NSLog(@"%@ ", [text substringFromIndex:text.length/2]);
    ...
    NSArray* arrayOfSentences = [text componentsSeparatedByString:@". "];
    
//    NSLog(@"%@", arrayOfSentences);
    
    return arrayOfSentences;
    
}

- (NSMutableArray*)studentWithArrayOfSentences:(NSArray*) arrayOfSentences {
    
    NSMutableArray* sentencesWithNSString = [[NSMutableArray alloc] init];
    
    for (NSString* sentence in arrayOfSentences) {
        
        if ([sentence containsString:@"NSString"]) {
            [sentencesWithNSString addObject:sentence];
        }
        
    }
    
//    NSLog(@"%@", sentencesWithNSString);
    
    return sentencesWithNSString;
    
}

- (void)masterWithSentencesWithNSString:(NSMutableArray*) sentencesWithNSString {
    
    NSMutableArray* reverseSentences = [[NSMutableArray alloc] init];
    
    for (NSString* sentence in sentencesWithNSString) {
        
        
        NSArray* arrayOfWords = [sentence componentsSeparatedByString:@" "];
        arrayOfWords = [[arrayOfWords reverseObjectEnumerator] allObjects];
        NSString* reverseSentence = [arrayOfWords componentsJoinedByString:@" "];
        [reverseSentences addObject:reverseSentence];
        
    }

//    NSLog(@"%@",reverseSentences);
...
- (void)superman {
    ...
    NSString* text = [[self getText] lowercaseString];
    
    NSString* alphabet = [NSString stringWithFormat:@"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"];
    
    for (int i; i <= text.length - 1; i++) {
        
        if (![alphabet containsString:[text substringWithRange:NSMakeRange(i, 1)]]) {
            text = [text stringByReplacingCharactersInRange:NSMakeRange(i - 1, 1) withString:[[text substringWithRange:NSMakeRange(i - 1, 1)] uppercaseString]];
        }
